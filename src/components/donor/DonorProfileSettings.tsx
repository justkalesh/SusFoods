...
export function DonorProfileSettings() {
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      if (org) {
        ...
        // No org row — populate from user metadata (self-heal)
        setHasOrgRow(false)
        setProfile({
          businessName: meta.org_name || meta.organization_name || "",
          businessType: meta.org_type || "",
          contactName: contactName || "",
  ...
  const handleSave = async () => {
    ...
    if (hasOrgRow) {
      ...
      // Create new row with full data from metadata
      const result = await supabase
        .from("organizations")
        .insert({
          ...payload,
          legal_name: meta.org_legal_name || `SUS-BIZ-${new Date().getFullYear()}-${Math.floor(10000 + Math.random() * 90000)}`,
    ...
    <Card className="bg-white border-slate-200 shadow-lg">
      ...
      <CardContent className="pt-8 space-y-8">
        ...
        <div className="flex justify-center">
          ...
          >
            <div className="w-full h-full rounded-full bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="dp-businessName" value={profile.businessName} onChange={e => update("businessName", e.target.value)} placeholder="Your business name" className="h-11 pl-10 bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus-visible:ring-emerald-500/50 focus-visible:border-emerald-500 transition-all" />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="dp-contactName" value={profile.contactName} onChange={e => update("contactName", e.target.value)} placeholder="Contact person name" className="h-11 pl-10 bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus-visible:ring-emerald-500/50 focus-visible:border-emerald-500 transition-all" />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="dp-phone" type="tel" value={profile.phone} onChange={e => update("phone", e.target.value)} placeholder="+91 XXXXX XXXXX" className="h-11 pl-10 bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus-visible:ring-emerald-500/50 focus-visible:border-emerald-500 transition-all" />
            </div>
          </div>
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <div className="relative">
              ...
              <Input id="dp-address" value={profile.address} onChange={e => update("address", e.target.value)} placeholder="Your business address" className="h-11 pl-10 bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus-visible:ring-emerald-500/50 focus-visible:border-emerald-500 transition-all" />
            </div>
          </div>
        </div>
